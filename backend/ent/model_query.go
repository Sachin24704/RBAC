...
func (mq *ModelQuery) QueryOperations() *OperationQuery {
	query := (&OperationClient{config: mq.config}).Query()
...
func (mq *ModelQuery) Clone() *ModelQuery {
	...
	return &ModelQuery{
		...
		withOperations: mq.withOperations.Clone(),
...
func (mq *ModelQuery) WithOperations(opts ...func(*OperationQuery)) *ModelQuery {
	query := (&OperationClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withOperations = query
	return mq
}
...
func (mq *ModelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Model, error) {
	...
	if query := mq.withOperations; query != nil {
		if err := mq.loadOperations(ctx, query, nodes,
			func(n *Model) { n.Edges.Operations = []*Operation{} },
			func(n *Model, e *Operation) { n.Edges.Operations = append(n.Edges.Operations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *ModelQuery) loadOperations(ctx context.Context, query *OperationQuery, nodes []*Model, init func(*Model), assign func(*Model, *Operation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(operation.FieldModelID)
	}
	query.Where(predicate.Operation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(model.OperationsColumn), fks...))
	}))
