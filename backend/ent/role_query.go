...
func (rq *RoleQuery) QueryRoleOperations() *RoleOperationQuery {
	query := (&RoleOperationClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withRoleOperations: rq.withRoleOperations.Clone(),
...
func (rq *RoleQuery) WithRoleOperations(opts ...func(*RoleOperationQuery)) *RoleQuery {
	query := (&RoleOperationClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoleOperations = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withRoleOperations; query != nil {
		if err := rq.loadRoleOperations(ctx, query, nodes,
			func(n *Role) { n.Edges.RoleOperations = []*RoleOperation{} },
			func(n *Role, e *RoleOperation) { n.Edges.RoleOperations = append(n.Edges.RoleOperations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadRoleOperations(ctx context.Context, query *RoleOperationQuery, nodes []*Role, init func(*Role), assign func(*Role, *RoleOperation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(roleoperation.FieldRoleID)
	}
	query.Where(predicate.RoleOperation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.RoleOperationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
