...
// The RoleOperationFunc type is an adapter to allow the use of ordinary
// function as RoleOperation mutator.
type RoleOperationFunc func(context.Context, *ent.RoleOperationMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RoleOperationFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RoleOperationMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RoleOperationMutation", m)
}
